...
#
#Modified version request but it does not give the response res
#Mainly used when exporting the STL to avoing showing the data on screen
#

#*********************************************************************************

    def request_no_out(self, method, path, query={}, headers={}, body={}, base_url=None):
        '''
        Issues a request to Onshape but suppress the output text if the request succeeded
        ...
        if res.status_code == 307:
            ...
            return self.request_no_out(method, location.path, query=new_query, headers=headers, base_url=new_base_url)
        elif not 200 <= res.status_code <= 206:
            if self._logging:
                utils.log('request failed, details: ' + res.text, level=1)
        else:
            if self._logging:
                #utils.log('request succeeded, details: ' + res.text)
                utils.log('request succeeded, details: ' + 'Joegi suppressed the out - Too much text')
